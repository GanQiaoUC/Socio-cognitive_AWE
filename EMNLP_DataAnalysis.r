...
data$Ethnicity <- factor(data$Ethnicity, levels = c("Asian","African","Indian","Latino","White", "Mixed"))
data$Education <- factor(data$Education, levels = c("PhD", "Prof","Mas","Bach","Coll","HS","LowHS"))
...
model_scaled2 <- lmer(Score ~ ScoreType*(Age + Gender + Ethnicity + Nativeness + Education + Topic + P_burst+R_burst+ MeanPause+
