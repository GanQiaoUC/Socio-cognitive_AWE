...
essay_folder = os.path.expanduser("change to yours")
...
# Get multiple scores per essay
def rate_essay_multiple(essay, n=2):
        ...
        try:
            ...
            try:
                ...
                if 1 <= score <= 6:
                    ...
                    print(f"Out-of-range score received: {score}")
            except ValueError:
                print(f"Invalid score received: '{score_text}'")
...
# Rate essays
essay_scores = {}
for filename, essay in essays.items():
    scores = rate_essay_multiple(essay, n=2)
    essay_scores[filename] = scores
    print(f"{filename}: {scores}")

# Save raw scores
with open("essay_raw_scores.json", "w", encoding="utf-8") as f:
    json.dump(essay_scores, f, indent=4)
